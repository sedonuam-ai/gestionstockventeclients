...
function addMovement({ articleId, type, quantity, pau, date }) {
  ...
  if (type === "entree") {
    const p = Number(pau);
    if (p == null || p < 0 || Number.isNaN(p)) return toast("P.A.U invalide.", "error");
    const newQty = article.qty + qty;
    const newPMP = newQty > 0 ? (article.qty * article.pmp + qty * p) / newQty : 0;
    data.movements.push({ id: uid(), articleId, type, date: date || todayISO(), quantity: qty, pau: p, total: qty * p, pmpAfter: newPMP, qtyAfter: newQty });
    article.qty = newQty; article.pmp = newPMP;
    saveData(); toast("Entrée de stock enregistrée."); render();
  } else {
    if (qty > article.qty) return toast("Stock insuffisant pour cette sortie.", "error");
    const newQty = article.qty - qty;
    data.movements.push({ id: uid(), articleId, type, date: date || todayISO(), quantity: qty, pau: article.pmp, total: qty * article.pmp, pmpAfter: article.pmp, qtyAfter: newQty });
    article.qty = newQty;
    saveData(); toast("Sortie de stock enregistrée."); render();
  }
}
...
function addSale({ articleId, quantity, date }) {
  const qty = Number(quantity);
  const article = data.articles.find((a) => a.id === articleId);
  if (!article) return toast("Article introuvable.", "error");
  if (!qty || qty <= 0) return toast("Quantité invalide.", "error");
  if (qty > article.qty) return toast("Stock insuffisant pour cette vente.", "error");

  const puVente = article.pmp * (1 + MARKUP);
  const totalHT = puVente * qty;
  const tva = totalHT * TVA_RATE;
  const totalTTC = totalHT + tva;
  const saleDate = date || todayISO();
  const sale = { id: uid(), articleId, date: saleDate, quantity: qty, pmp: article.pmp, puVente, totalHT, tva, totalTTC };
  ...
  article.qty -= qty;
  saveData(); toast("Vente enregistrée."); render();
}
function resetStock() {
...
function movementTypeLabel(m) {
  if (m.type === "entree") return `<span style="color:#1F4D3A;font-weight:600">Entrée</span>`;
  if (m.type === "correction") return `<span style="color:#8A5A12;font-weight:600">Correction</span>`;
  return `<span style="color:#A63D2F;font-weight:600">Sortie${m.saleId ? " (vente)" : ""}</span>`;
}
  ...
  const recentHtml = recent.length === 0 ? emptyState("down", "Aucun mouvement enregistré.") :
    `<ul style="list-style:none;margin:0;padding:0">${recent.map((m) => {
      ...
      return `<li style="display:flex;justify-content:space-between;font-size:13.5px;border-bottom:1px solid var(--border);padding:8px 0">
        ...
        <span class="mono" style="font-size:12px;color:#5c5646">${m.type === "entree" ? "+" : "−"}${m.quantity} · ${fmtDate(m.date)}</span>
      </li>`;
    }).join("")}</ul>`;
...
function viewArticles() {
  ...
  const formHtml = families.length === 0 ? `<p style="font-size:13.5px;color:#8A5A12">Créez d'abord une famille de produits.</p>` :
...
function viewMouvements() {
  ...
  const formHtml = arts.length === 0 ? `<p style="font-size:13.5px;color:#8A5A12">Enregistrez d'abord un article.</p>` :
    `<form id="movForm">
      <div style="display:flex;gap:8px;margin-bottom:16px">
        ...
        <button type="button" class="btn" data-mvtype="sortie" style="flex:1;justify-content:center;border:2px solid var(--border);background:transparent;color:var(--muted)">${icon("up")} Sortie</button>
      </div>
      ...
      <div class="grid grid-4">
        ...
        <div id="mvPauWrap">${field("P.A.U (prix d'achat)", `<input class="input" type="number" min="0" step="any" name="pau" />`)}</div>
  ...
  const tableHtml = recent.length === 0 ? emptyState("down", "Aucun mouvement.") :
    `<table class="ledger"><thead><tr><th>Date</th><th>Article</th><th>Type</th><th class="tr-right">Quantité</th><th class="tr-right">P.A.U / P.M.P</th><th class="tr-right">Prix total</th><th class="tr-right">Solde</th></tr></thead>
    <tbody>${recent.map((m) => {
      const art = data.articles.find((a) => a.id === m.articleId);
...
function viewModifier() {
  const arts = articlesWithFamily();
  if (arts.length === 0) return sectionHead("Gestion stock", "Modifier le stock") + panel("Correction manuelle d'un article", `<p style="font-size:13.5px;color:#8A5A12">Aucun article enregistré.</p>`);
...
function viewVente() {
  const arts = articlesWithFamily();
  if (arts.length === 0) return sectionHead("Gestion ventes", "Prix unitaire de vente = P.M.P majoré de 85 % · T.A.V 18 %") + panel("Nouvelle vente", `<p style="font-size:13.5px;color:#8A5A12">Aucun article disponible.</p>`);
  ...
  if (article) {
    const puVente = article.pmp * (1 + MARKUP);
    infoHtml = `<div class="box grid grid-4" style="margin-top:14px">
      <div><div class="lbl" style="margin-bottom:4px">P.M.P</div>${fmt(article.pmp)}</div>
  ...
  return sectionHead("Gestion ventes", "Prix unitaire de vente = P.M.P majoré de 85 % · T.A.V 18 %") +
...
function viewHistoriqueVentes() {
  ...
  const body = sorted.length === 0 ? emptyState("receipt", "Aucune vente enregistrée.") :
    `<table class="ledger"><thead><tr><th>Date</th><th>Article</th><th class="tr-right">Qté</th><th class="tr-right">P.M.P</th><th class="tr-right">P.U. Vente</th><th class="tr-right">Total HT</th><th class="tr-right">TVA</th><th class="tr-right">Total TTC</th></tr></thead>
    <tbody>${sorted.map((s) => {
      ...
      <td class="tr-right mono">${fmt(s.pmp)}</td><td class="tr-right mono">${fmt(s.puVente)}</td><td class="tr-right mono">${fmt(s.totalHT)}</td>
      <td class="tr-right mono">${fmt(s.tva)}</td><td class="tr-right mono" style="font-weight:700">${fmt(s.totalTTC)}</td></tr>`;
    }).join("")}</tbody>
    <tfoot><tr><td colspan="6" class="tr-right" style="text-transform:uppercase;font-size:11px">Totaux</td><td class="tr-right mono">${fmt(totalTVA)}</td><td class="tr-right mono" style="font-size:16px">${fmt(totalTTC)}</td></tr></tfoot></table>`;
...
const VIEWS = {
  ...
  vente: viewVente, "historique-ventes": viewHistoriqueVentes,
};
...
function wireSectionEvents() {
  ...
  if (movForm) {
    ...
    function refreshTypeUI() {
      typeBtns.forEach((b) => {
        ...
        b.style.borderColor = on ? (mvType === "entree" ? "#1F4D3A" : "#A63D2F") : "#D8D2B8";
        b.style.background = on ? (mvType === "entree" ? "rgba(31,77,58,.1)" : "rgba(166,61,47,.1)") : "transparent";
        b.style.color = on ? (mvType === "entree" ? "#1F4D3A" : "#A63D2F") : "#8A8266";
      });
      typeInput.value = mvType;
      const art = data.articles.find((a) => a.id === articleSel.value);
      pauWrap.innerHTML = mvType === "entree"
        ? field("P.A.U (prix d'achat)", `<input class="input" type="number" min="0" step="any" name="pau" id="mvPau" />`)
    ...
    function refreshInfo() {
      ...
      const p = mvType === "entree" ? Number(pauInput?.value || 0) : art.pmp;
      infoBox.innerHTML = `<div class="box" style="display:flex;justify-content:space-between"><span>Stock actuel : ${art.qty} u. · P.M.P ${fmt(art.pmp)}</span><span style="font-weight:700">Prix total : ${fmt(qty * p)}</span></div>`;
    }
    ...
    movForm.addEventListener("input", (e) => { if (e.target.name === "quantity" || e.target.name === "pau") refreshInfo(); });
    refreshTypeUI();

    movForm.addEventListener("submit", (e) => {
      e.preventDefault();
      const fd = new FormData(movForm);
      addMovement({ articleId: fd.get("articleId"), type: fd.get("type"), quantity: fd.get("quantity"), pau: fd.get("pau"), date: fd.get("date") });
    });
  }
  ...
  if (venteForm) {
    ...
    function refresh() {
      ...
      const totalHT = puVente * qty, tva = totalHT * TVA_RATE, totalTTC = totalHT + tva;
    ...
    venteForm.addEventListener("submit", (e) => {
      ...
      addSale({ articleId: venteArticleId, quantity: fd.get("quantity"), date: fd.get("date") });
    });
  }
}
